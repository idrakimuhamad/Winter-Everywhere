...
<CodeSurfer
  ...
  steps={[
    ...
    { range: [8, 15], notes: "Get he data based on the given argument" },
...
---
### But how to query on same field with different arguments?
---
<CodeSurfer
  title="Aliases"
  notes=""
  ...
  dark={false}
  steps={[
    { lines: [2,5], notes: "We can now query for two human, with different arguments" },
...
<CodeSurfer
  ...
  dark={false}
  steps={[
    { lines: [2,5], notes: "Assuming we would want to display data of 2 heroes at the same time, where all of the query would likely be a repetitive fields" },
...
<CodeSurfer
  ...
  dark={false}
  steps={[
    { lines: [1], notes: "We create a mutation operation called CreateReviewForEpisode, which accept 2 variables, the episode and the review we are writing" },
    { lines: [2], notes: "This is mutation type, which will use the variables pass on by the operation" },
    { range: [3, 4], notes: "Notice that we include the field of the of the newly created review. This is useful when you're mutating into an existing data, so you can get the changes in one request" },
...
<CodeSurfer
  ...
  steps={[
    ...
    { lines: [4], notes: "Todos just a component we created that will render our Todo list" },
    { range: [6, 8], notes: "Set up the connection with the backend" },
    { lines: [15, 32], notes: "Apollo Provider is the connector between Apollo Client and React, much like Redux's Provider" },
    { lines: [28], notes: "We can then use Apollo in our component" },
  ]}
/>
---
<CodeSurfer
  ...
  steps={[
    ...
    { lines: [4], notes: "Todo is a component that represent a single item of our todo" },
    { lines: [7, 30], notes: "Query is a component provided by apollo client which you can use to construct a GraphQL query" },
    { range: [8, 18], notes: "Similar like what we saw earlier, you can write the GraphQL query using the graphql-tag" },
    { range: [10, 16], notes: "We want to get all todos and all the fields" },
    { lines: [19], notes: "The Query component provided a callback with a some useful argument, such as loading, error and the actual data" },
    { lines: [20], notes: "Which we can use to show loading UI..." },
    { lines: [21], notes: "...Or to show an eror message when data loading failed" },
    { range: [24, 30], notes: "We can then use the data to form our Todo list, which we supply to our Todo component to do its rendering job" },
  ]}
/>
---
<CodeSurfer
  ...
  steps={[
    ...
    { range: [5, 23], notes: "As seen on Query, you can also use the same graphql-tag to write the mutation query." },
    { lines: [29, 30, 31, 32, 33, 34, 35, 48], notes: "Similar to Query, Mutation is a component provided by Apollo Client." },
    { lines: [30], notes: "POST_MUTATION is the mutation script we wrote earlier" },
    { range: [31, 35], notes: "Variables can be pass through the variable prop" },
    { lines: [36, 47], notes: "The component provide a function with an argument for rendering which can be use to trigger the mutation" },
    { lines: [41], notes: "We only need to bind it to an event handler and use the props" }
  ]}
/>
---
...
Get the deck at https://github.com/idrakimuhamad/Winter-Everywhere. Can also look in Confluence later on for write up.
